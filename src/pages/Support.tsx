...
const Support = () => {
  ...
  const agent = useMemo(() => ({
    ...
    avatar: pickRandom(AGENT_AVATARS),
  }), []);
      ...
      <div className="flex-1 overflow-y-auto px-4 py-4 space-y-3" style={{ paddingBottom: '80px' }}>
        {messages.map((msg) => (
          ...
          >
            ...
            {msg.from === 'bot' && (
              <div
                className="w-8 h-8 rounded-full flex items-center justify-center text-base shrink-0 mb-0.5"
                style={{ background: 'linear-gradient(135deg,#FF8A00,#FF6A00)', color: '#fff' }}
              >
                {agent.avatar}
              </div>
            )}
